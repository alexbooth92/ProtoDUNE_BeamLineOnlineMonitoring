...
int main(int argc, char *argv[])
{
  ...
  if(vec_DataPath.size()!=0)
  {
    ...
    //beamline.printPROFCoincidencesUnique(map_Detector);
    beamline.printTFPROFCoincidencesDegenerate(map_Detector);
    ...
    //std::vector<CombinedCoincidenceRecord::TFPROFCoincidence> vec_TFPROFCO_Degen   = beamline.getTFPROFCoincidencesDegenerate();
    //std::vector<CombinedCoincidenceRecord::TFPROFCoincidence> vec_TFPROFCO_Uniq    = beamline.getTFPROFCoincidencesUnique();

    std::vector<TH1D*> vec_PROFHist1D;
    std::vector<TH2D*> vec_PROFHist2D;
    std::vector<TH1D*> vec_TFHist1D;
    std::vector<TH2D*> vec_TFHist2D;
    std::vector<TH1D*> vec_TFPROFHist1D;
    std::vector<TH2D*> vec_TFPROFHist2D;
    ...
    //plotTFPROF(vec_TFPROFCO_Uniq,  vec_TFPROFHist1D, vec_TFPROFHist2D, "UNIQ" );
    //plotTFPROF(vec_TFPROFCO_Degen, vec_TFPROFHist1D, vec_TFPROFHist2D, "DEGEN");
    ...
    //beamline.dumpTFPROFCoincidencesDegenerate(map_Detector,(TString)s_OutDir+"Analyse_BIBL_Tree_"+(TString)s_FirstTime+"_"+(TString)s_LastTime+".root");
    //beamline.dumpTFPROFCoincidencesUnique    (map_Detector,(TString)s_OutDir+"Analyse_BIBL_Tree_"+(TString)s_FirstTime+"_"+(TString)s_LastTime+".root");
    ...
    TDirectory *dir_TFPROF = f_Out->mkdir("TFPROF");
    dir_TFPROF->cd();
    for(auto hist : vec_TFPROFHist1D){hist->Write();}
    for(auto hist : vec_TFPROFHist2D){hist->Write();}
