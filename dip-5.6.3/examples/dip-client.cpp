...
public:
	...
	* */
	Client(const int argc, const char ** argv)
	{
		  ...
		  dip->setDNSNode("dipnstn1");
         
		  
    std::cout << "number of pubs " << numberOfPubs << std::endl; 
		  for(int i = 0 ; i < numberOfPubs ; i++){
		      ostringstream oss;
		       oss << "dip/NeutrinoPlatform/NP04/DCS/Temp" << "_" << i;
		       sub[i] = dip->createDipSubscription(oss.str().c_str(), handler);
         std::cout << "creating dip subscription" << std::endl;
		  }
